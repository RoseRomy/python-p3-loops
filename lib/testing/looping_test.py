...
class TestFizzBuzz:
    ...
    def test_prints_1_to_100_fizzbuzz(self):
        ...
        answer = captured_out.getvalue()
        assert len(
            answer) != 0, "Nothing printed! Check your loop condition. Also do you have print statements?"
        ...
        for line in answer.split('\n'):
            if (line):  # answer.split(\n) produces a list that ends in ''
                if i % 15 == 0:
                    assert line == "FizzBuzz", f"Should have printed 'Buzz' when number is {i}, got {line} instead"
                elif i % 3 == 0:
                    ...
                    assert str(
                        i) == line, f"Should have printed {i}, got {line} instead"
                i += 1
